...
const { wait, getFileExtension } = require('../utils');
  ...
  try {
    ...
    if(response.status === 200) {
      ...
      const post_list = $('div.recommend_underList').children('div').each(function() {
        ...
        if(post_hit > 35000 && titleLength === 3) {
      ...
      for(const url of urls) {
        ...
        if(response.status === 200) {
          ...
          $('#div_content > div.post_view > div.post_content > article > div').children('p').each(function() {
            ...
            if(img_url) {
              const fileExtension = getFileExtension(img_url);
              uploadToS3(img_url);
              const s3_url = AWS_S3_BUCKET_URL + uuidv5(img_url, uuidv5.URL) + fileExtension;
